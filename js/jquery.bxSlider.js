...
//			$children = $parent.children(options.childSelector);
			$children = $parent.children('li '+options.childSelector);
			$outerWrapper = '';
		...
		function showPager(type){
			...
			// if not, use default pager
				...
				for(var i=1; i<=pagerQty; i++){
					pagerString += '<a href="" id="bx-ex-pager-'+(i-1)+'" class="pager-link pager-'+i+'">'+i+'</a>';
				}
			}else if(type == 'short') {
			...
			if(options.pagerSelector){
				...
				var $pagerContainer = $('<div class="bx-pager"><span>&nbsp;</span></div>');
			...
			$pager.children().click(function() {
				...
				if(options.pagerType == 'full'){
					...
					var slideIndex = 0;
					if($(this).attr('id')){
						var num=Number($(this).attr('id').substring(12));
						if(typeof(num)=="number") slideIndex=num;
					}
			...
			if(isMouseOver)
				if(!options.infiniteLoop && options.hideControlOnEnd){
					...
					if(currentSlide == firstSlide){
						// $outerWrapper.children('.bx-prev').fadeOut(200);			
						$outerWrapper.children('.bx-prev').hide();			
					}else{
						// $outerWrapper.children('.bx-prev').fadeIn(0);
					...
					if(currentSlide >= lastPossibleSlide){
						// $outerWrapper.children('.bx-next').fadeOut(200);
						$outerWrapper.children('.bx-next').hide();
					}else{
						// $outerWrapper.children('.bx-next').fadeIn(0);
		...
		function getSlidePosition(number, side){			
			...
			if(typeof($slidePosition)=='undefined') return 0;
		...
		// add connecty custom
		if(options.actKeyObj){
			options.actKeyObj.hover(function(){
				// $outerWrapper.children('.bx-prev').stop();
				// $outerWrapper.children('.bx-next').stop();
				isMouseOver=true;
				checkEndControls();
			},function(){
				isMouseOver=false;
